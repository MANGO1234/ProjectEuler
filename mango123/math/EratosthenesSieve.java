...
public class EratosthenesSieve implements PrimeSearcher {
	...
	public int nthPrime(int n) {
		if (n < 1 || n > numOfPrimesInSieve) return -1; //n need to be within 0 and numOfPrimesInSieve
		if (n == 1) return 2;							//account manually for n = 1 which returns 2

		for (int i = 0, len = sieve.length, count = 1 /* count = 1 for not including 2 in the array */; i < len; ++i) {
			if (sieve[i]) {
				if (++count >= n) return (i << 1) + 3; //i * 2 + 3 to account for using only odd numbers from 3
			}
		}
	...
	public boolean isPrime(int n) {
		...
		if (n == 2) return true;
		else if ((n & 1) == 0 || n < 2) return false;

		
		//if it's within the sieve, just return the value inside the sieve
		else if (n <= sieveSize) return sieve[(n >> 1) - 1];
	...
	private boolean testLargeNumber(int number) {
		...
		for (int i = 0; i < bound; ++i) {
			if (sieve[i] && (number % ((i << 1) + 3)) == 0) return false;
		}
		return true;
	}
