...
public class Euler {
	...
	public static String hm(Function<String, Function<Integer, String>> f) {
		return f.apply("omg").apply(2);
	}
	...
	public static List<Long> getAll(int p, int n, int d, int dCount, long acc) {
		...
		for (int i = 0; i < 10; i++) {
			...
			if (maxT >= -1 && minT <= -1) {
				List<Long> ls = getAll(p - 1, i, d, i == d ? dCount + 1 : dCount, acc);
				for (Long x : ls) {
